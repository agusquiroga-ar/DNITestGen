...
class NewDniGenerator {
  ...
  static String generateUrl(Identity identity) {
    ...
    final fechaEmision = "${identity.fechaEmision.year.toString().padLeft(4, '0')}-${identity.fechaEmision.month.toString().padLeft(2, '0')}-${identity.fechaEmision.day.toString().padLeft(2, '0')}";
  ...
  static Widget buildQrWidget(String url) {
    return QrImageView(
      data: url,
      version: 10, // Forzado a versión 10 (alta densidad) como indica la especificación
